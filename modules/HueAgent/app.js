...
function sendMethodResponse(response, statusCode, body, methodName) {
  response.send(statusCode, body, (err) => {
    if (err) {
        logger.logError(`Failed sending ${methodName} response: ${err}`);
    } else {
...
function connectWithRetry(retryCount = 0) {
  ModuleClient.fromEnvironment(Transport, (err, client) => {
    if (err) {
        logger.logError(`[HueAgent] Error creating client: ${err}`);
    ...
    client.on("error", (clientError) => {
        logger.logError(`[HueAgent] Client error: ${clientError}`);
    });

    client.open((openErr) => {
      if (openErr) {
          logger.logError(`[HueAgent] Error opening connection: ${openErr}`);
        if (retryCount < MAX_RETRIES) {
          logger.logInfo(`[HueAgent] Retrying connection in ${RETRY_DELAY_MS}ms... (attempt ${retryCount + 1}/${MAX_RETRIES})`);
          setTimeout(() => connectWithRetry(retryCount + 1), RETRY_DELAY_MS);
        } else {
      ...
      (async () => {
        try {
          ...
          if (hueBridge) {
            ...
            logger.logWarn(`[HueAgent] Failed to load Hue bridge credentials: ${error.message}`);
        }
      })();
      ...
      client.onMethod("initialize", async (request, response) => {
        ...
        try {
          ...
          sendMethodResponse(response, 200, { bridgeIp: hueBridge.bridgeIp, username: hueBridge.username }, 'initialize');
            ...
            logger.logError(`[HueAgent] Hue pairing failed: ${error.message}`);
      ...
      client.onMethod("startMonitoring", async (request, response) => {
        try {
          ...
          sendMethodResponse(response, 200, { status: "monitoring started" }, 'startMonitoring');
            ...
            logger.logError(`[HueAgent] Start monitoring failed: ${error.message}`);
      ...
      client.onMethod("stopMonitoring", async (request, response) => {
        try {
          ...
          sendMethodResponse(response, 200, { status: "monitoring stopped" }, 'stopMonitoring');
            ...
            logger.logError(`[HueAgent] Stop monitoring failed: ${error.message}`);
        ...
        try {
          ...
          try {
            ...
            client.sendOutputEvent(HUE_EVENTS_OUTPUT, message, (err) => {
              if (err) {
                  logger.logError(`[HueAgent] Failed to send snapshot: ${err.message}`);
              } else {
            ...
            logger.logError(`[HueAgent] Snapshot failed: ${error.message}`);
